...
class LicenseBoxAPI{
    ...
    public function __construct(){
        $this->product_id = '58BC3A4A';
        $this->api_url = 'http://wael-m.test/';
        $this->api_key = 'D48326ED953303F42ED7';
    ...
    public function verify_license($time_based_check = false, $license = false, $client = false){
        ...
        if($time_based_check && $this->verification_period > 0){
            ...
            $today = date('d-m-Y');
            if(empty($_SESSION["ab2744e423ca5af"])){
                $_SESSION["ab2744e423ca5af"] = '00-00-0000';
            }
            if($type == 1){
                $type_text = '1 day';
            }elseif($type == 3){
            ...
            if(strtotime($today) >= strtotime($_SESSION["ab2744e423ca5af"])){
                $get_data = $this->call_api(
                    'POST',
